...
class PlaybackNotifier extends StateNotifier<PlaybackState> {
  ...
  Future<void> removeQueueAt(int index) async {
    ...
    if (current != null && !newQueue.any((s) => s.id == current.id)) {
      await _handler.setQueue(newQueue, initialIndex: index.clamp(0, newQueue.length - 1));
      return;
    }
    final newIndex = newQueue.indexWhere((s) => s.id == current?.id);
    await _handler.setQueue(newQueue, initialIndex: newIndex < 0 ? 0 : newIndex);
  }
  ...
  Future<void> moveQueueItem(int oldIndex, int newIndex) async {
    ...
    final currentIndex = list.indexWhere((s) => s.id == currentId);
    await _handler.setQueue(list, initialIndex: currentIndex < 0 ? 0 : currentIndex);
  }
